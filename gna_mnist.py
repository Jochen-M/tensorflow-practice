...
class GNA(object):
    def __init__(self, n_input, n_hidden, transfer_function=tf.nn.softplus,
        ...
        self.transfer_fucntion = transfer_function
        ...
        self.hidden = self.transfer_fucntion(tf.add(tf.matmul(
    ...
    def _initialize_weights(self):
        all_weights = dict()
        all_weights['w1'] = self.xavier_init(self.n_input, self.n_hidden)
        all_weights['b1'] = tf.Variable(tf.zeros(
            [self.n_hidden], dtype=tf.float32))
        all_weights['w2'] = tf.Variable(tf.zeros(
            [self.n_hidden, self.n_input], dtype=tf.float32))
        all_weights['b2'] = tf.Variable(tf.zeros(
...
if __name__ == '__main__':
    ...
    from tools import standard_scale, get_random_block_from_data
    ...
    autoencoder = GNA(
        784, 200, optimizer=tf.train.AdamOptimizer(learning_rate=0.001), scale=0.01)
    ...
    print("Total cost: " + str(autoencoder.calc_total_cost(X_test) / mnist.test.num_examples * batch_size))
